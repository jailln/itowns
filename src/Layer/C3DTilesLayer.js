...
class C3DTilesLayer extends GeometryLayer {
    ...
    constructor(id, config, view) {
        ...
        const tilesRenderer = new TilesRenderer(this.source.url);
        tilesRenderer.manager.addHandler(/\.gltf$/, loader);

        tilesRenderer.setCamera(view.camera.camera3D); // we should not have the view here
        tilesRenderer.setResolutionFromRenderer(view.camera.camera3D, view.mainLoop.gfxEngine.renderer);
        tilesRenderer.onLoadTileSet = function (scene) {
            view.notifyChange();
        };
        tilesRenderer.onLoadModel = function (scene) {
            view.notifyChange();
        };
        view.scene.add(tilesRenderer.group); // TODO: should not be here
        view.mainLoop.gfxEngine.tilesRenderer = tilesRenderer; // DIRTY

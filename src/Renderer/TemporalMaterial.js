/*
 * This Material is used for temporal 3D-Tiles. It allows to create a shader
 * to do culling based on temporal values (display date and year of construction /
 * year of demolition of each city object)
 */

import * as THREE from 'three';
import TemporalTileVS from './Shader/TemporalTileVS.glsl';
import TemporalTileFS from './Shader/TemporalTileFS.glsl';
...
const TemporalMaterial = function TemporalMaterial(options) {
    ...
    this.fragmentShader = this.fragmentShaderHeader + TemporalTileFS;
    this.vertexShader = PrecisionQualifier + vsOptions + TemporalTileVS;
    ...
    this.uniforms.lightingEnabled = new THREE.Uniform(true);

    this.uniforms.noTextureColor = new THREE.Uniform(new THREE.Color(0.8, 0.8, 0.8));
...
TemporalMaterial.prototype = Object.create(THREE.RawShaderMaterial.prototype);
TemporalMaterial.prototype.constructor = TemporalMaterial;

TemporalMaterial.prototype.setupTemporalUniforms = function setupTemporalUniforms(batchLength, discardTemporal) {
    this.fragmentShader = `const int DISCARD_SIZE = ${batchLength};\n ${this.fragmentShader}`;
    this.uniforms.discardTemporal = new THREE.Uniform(discardTemporal);
};

TemporalMaterial.prototype.dispose = function dispose() {
...
TemporalMaterial.prototype.setSequence = function setSequence(sequenceLayer) {
    let offsetLayer = 0;
    let offsetTexture = 0;
...
TemporalMaterial.prototype.removeColorLayer = function removeColorLayer(layer) {
...
TemporalMaterial.prototype.setTexturesLayer = function setTexturesLayer(textures, layerType, layer) {
...
TemporalMaterial.prototype.setTexture = function setTexture(texture, layerType, slot, offsetScale) {
...
TemporalMaterial.prototype.setColorLayerParameters = function setColorLayerParameters(params) {
...
TemporalMaterial.prototype.pushLayer = function pushLayer(param) {
...
TemporalMaterial.prototype.indexOfColorLayer = function indexOfColorLayer(layerId) {
    return this.colorLayersId.indexOf(layerId);
};

TemporalMaterial.prototype.getColorLayersCount = function getColorLayersCount() {
    return this.colorLayersId.length;
};

TemporalMaterial.prototype.getTextureOffsetByLayerIndex = function getTextureOffsetByLayerIndex(index) {
    return this.uniforms.paramLayers.value[index].x;
};

TemporalMaterial.prototype.getTextureCountByLayerIndex = function getTextureCountByLayerIndex(index) {
    return this.layerTexturesCount[index];
};

TemporalMaterial.prototype.getLayerTextureOffset = function getLayerTextureOffset(layerId) {
    const index = this.indexOfColorLayer(layerId);
    return index > -1 ? this.getTextureOffsetByLayerIndex(index) : -1;
};

TemporalMaterial.prototype.setLightingOn = function setLightingOn(enable) {
    this.uniforms.lightingEnabled.value = enable;
};

TemporalMaterial.prototype.setLayerFx = function setLayerFx(index, fx) {
    this.uniforms.paramLayers.value[index].z = fx;
};

TemporalMaterial.prototype.setTextureOffsetByLayerIndex = function setTextureOffsetByLayerIndex(index, offset) {
    this.uniforms.paramLayers.value[index].x = offset;
};

TemporalMaterial.prototype.setLayerUV = function setLayerUV(index, idUV) {
    this.uniforms.paramLayers.value[index].y = idUV;
};

TemporalMaterial.prototype.getLayerUV = function setLayerUV(index) {
    return this.uniforms.paramLayers.value[index].y;
};

TemporalMaterial.prototype.setLayerOpacity = function setLayerOpacity(index, opacity) {
    if (this.uniforms.paramLayers.value[index])
        { this.uniforms.paramLayers.value[index].w = opacity; }
};

TemporalMaterial.prototype.setLayerVisibility = function setLayerVisibility(index, visible) {
    this.uniforms.visibility.value[index] = visible;
};

TemporalMaterial.prototype.setLayerTexturesCount = function setLayerTexturesCount(index, count) {
    this.layerTexturesCount[index] = count;
};

TemporalMaterial.prototype.getLoadedTexturesCount = function getLoadedTexturesCount() {
    return this.loadedTexturesCount[l_ELEVATION] + this.loadedTexturesCount[l_COLOR];
};

TemporalMaterial.prototype.isColorLayerDownscaled = function isColorLayerDownscaled(layerId, zoom) {
...
TemporalMaterial.prototype.getColorLayerLevelById = function getColorLayerLevelById(colorLayerId) {
...
TemporalMaterial.prototype.getElevationLayerLevel = function getElevationLayerLevel() {
    return this.textures[l_ELEVATION][0].coords.zoom;
};

TemporalMaterial.prototype.getLayerTextures = function getLayerTextures(layerType, layerId) {
...
TemporalMaterial.prototype.setUuid = function setUuid(uuid) {
    this.uniforms.uuid.value = uuid;
};

TemporalMaterial.prototype.setFogDistance = function setFogDistance(df) {
    this.uniforms.distanceFog.value = df;
};

TemporalMaterial.prototype.setSelected = function setSelected(selected) {
    this.uniforms.selected.value = selected;
};


export default TemporalMaterial;

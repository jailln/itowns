...
uniform bool        discardTemporal[DISCARD_SIZE];
...
varying float       batchID; // batchId of the current feature
...
void main() {
    ...
    #endif
    {
        ...
        // Discard fragment for temporal reason
        for(int i = 0 ; i < DISCARD_SIZE; ++i)
        {
            if(i == int(floor(batchID + 0.5)))
            {
                if(discardTemporal[i] == true) {
                    discard;
                }
            }
        }
